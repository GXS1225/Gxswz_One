...
ImagePreview.prototype = {
  ...
  _filterShow: function() {
	...
	var   tmp   =   data.lastIndexOf("\\");   
    var   tmpstr   =   data.substring(tmp,data.length);   
    tmp   =   tmpstr.lastIndexOf(".");   
    tmpstr   =   tmpstr.substring(tmp,data.length-tmp);   
    tmpstr=tmpstr.toLowerCase();
	if(tmpstr==".jpg" || tmpstr==".bmp"  || tmpstr==".gif" || tmpstr==".png" ||tmpstr==".png")
	{	
		try{
		...
		//设置滤镜并显示
		document.getElementById("show_id_"+this.number+"_1").style.display = "none";  //
   		document.getElementById("show_id_"+this.number+"_2").style.display = "";  //
		
		this.img.style.filter = "progid:DXImageTransform.Microsoft.AlphaImageLoader(sizingMethod='scale',src=\"" + data + "\")";
		this._imgShow( ImagePreview.TRANSPARENT, preload.offsetWidth, preload.offsetHeight );
	}
	else{
		alert("对不起,您上传的图片格式不对,目前只支持gif,jpg,png,jpeg,bmp格式");	
		document.getElementById("imgview_apf_id_8_"+this.number).src=imgsrc ;
	}
  },
