...
function showPopupAdv(B) {
    ...
    this.init = function() {
        ...
        if (this.showclosed) {
            jQuery("#" + this.pdivId + "").html(jQuery("#" + this.pdivId + "").html() + "<img style='display:none;position:absolute;right:5px;top:6px;z-index:2;cursor:pointer;' src='../template/pc/main/default/images/float_ad_closed.gif' class='closedbutton' title='鍏抽棴' />")
        }
    ...
    if (this.adv.advType == 8) {
        ...
        jQuery(document.body).append("<div id='float_ad_replay'><a hrer='#' class='float_ad_replay' title='GXS</a><a hrer='#' class='float_ad_closed' title='鍏抽棴'>鍏抽棴</a></div>");
...
function showFullScreenAdv(A) {
    ...
    B.closedad = function() {
        ...
        function() {
            jQuery(this).hide();
            jQuery("#" + B.divId + " .fade_btn").show().html("閲嶆挱");
            B.pause = false
        })
    };
